...
#
##################################################
# Function: .LXsample_linear_2  
# Purpose : Samples from the LXSpline with a linear
#           B-Spline and 2 alphas
# Return 
#
#
##################################################
.LXsample_linear_2 <- function(t,y,JK,nsamps,nburn,anal_dtype,CONST,AXTREMA)#,NUM_CHG_PTS)
  {
  #################################
  ...
  CBX = shapespline(m.TREE[1,]-.5, t-0.5,2)
  #########################################
  #
  #Initial parameters
  ...
  ptree = 0.10
  tau = 0.25 
  x.Pred <- matrix(0,nrow=nrow(y),ncol=nsamps) #mean prediction of the data points
  lamT = 1
  intLAM = 0.001
  p = 0.01
 ...
 # print(omega)#*(1/omega))
  ###########################################
  # Adjacency matrices for the swap step 
  # for the simulated tempering
  ###########################################
  MT  <- 1:(length(JK)-1)
  ...
  for (i in 1:nsamps){
    ...
    if (runif(1) < 0.5){ # simple update 
      # sample which one of the simulated tempering
      # dispersions we are looking at and load the info
  ...
  #    if(NUM_CHG_PTS == 2)
      ...
      #################################################################
      # METROPOLIS STEP 3
      # SAMPLE A NEW KNOT IN THE TREE AND THUS A NEW x MATRIX
      #################################################################
      L = PROP.N.TREE(m.TREE);   POS = L$pos;   
      
      if (L$INSERT==TRUE){
        ######################################################################################
        # GET THE NEW SHAPESPLINE
        NCBX = shapesplineInsertLinear(as.matrix(L$m.TREE[1,]-0.5,nrow=1), as.matrix(t,ncol=1)-0.5,2,CBX,L$pos)
        NX = cbind(ONES,CONST*(NCBX[,,3] - (a1+a2)*NCBX[,,2]+(a1*a2)*NCBX[,,1]));
        NBETAS = matrix(0,nrow=ncol(NX),ncol=1); 
        T_IDX <- (POS-1):(POS+1)+1;
        NBETAS[-T_IDX] = BETAS[-((POS-1):(POS)+1)]; 
        yStar = augZ - NX[,-T_IDX]%*%NBETAS[-T_IDX];
        LAM = lamT; 
        N_R = sinsertBeta_bivariate(as.matrix(yStar,ncol=1), NX[,T_IDX,drop=FALSE], tau/KK, p,as.matrix(LAM),omega);
        ######################################################################################
        #Denominator Ration for the equivalent delete
        ######################################################################################
        T_IDX <- (POS-1):(POS)+1;
        yStar = augZ - X[,-T_IDX,drop=FALSE]%*%BETAS[-T_IDX,,drop=FALSE];
        LAM = lamT;
        C_R =  sdeleteBeta_bivariate(yStar,X[,T_IDX,drop=FALSE], tau/KK, p,as.matrix(LAM),omega);
        ##################################################################################
        lnLike = N_R$LPROB; 
        ldLike = C_R$LPROB;
      ...
      if (L$INSERT==FALSE){
        ...
        # GET THE NEW SHAPESPLINE
        NCBX = shapesplineDeleteLinear(as.matrix(L$m.TREE[1,]-0.5,ncol=1),as.matrix(t,ncol=1)-0.5,as.integer(2),CBX,as.integer(L$pos))
        NX = cbind(ONES,CONST*(NCBX[,,3] - (a1+a2)*NCBX[,,2]+(a1*a2)*NCBX[,,1]));
        ...
        LAM = lamT
        N_R = sdeleteBeta_bivariate(yStar,NX[,T_IDX,drop=FALSE],tau/KK,p,as.matrix(LAM),omega);
        #########################################################
        # Denomonator of the Ratio - Insert
        T_IDX <- (POS-1):(POS+1)+1;
        yStar = augZ - X[,-T_IDX,drop=FALSE]%*%BETAS[-T_IDX,,drop=FALSE];
        LAM = lamT
        C_R =  sinsertBeta_bivariate(yStar, X[,T_IDX,drop=FALSE], tau/KK, p,as.matrix(LAM),omega);
        #########################################################
        lnLike = N_R$LPROB; 
        ldLike = C_R$LPROB; 
      ...
      if ( test > 0 || runif(1)<exp(test))
      {
        ...
        CBX   = NCBX; 
        X = NX;   
        # now sample the new beta for the inserted knots
        if (L$INSERT){ 
          T_IDX <- (POS-1):(POS+1)+1;
